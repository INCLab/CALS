...
def check_time(time, minute, second, plus):
    time = int(time)
    minute = int(minute)
    second = int(second) + plus
    ...
    result_time = str(time)+':'+str(int(minute))+':'+str(round(second,3))
...
# ### Original code ###
# def eval_seq(opt, video_name, dataloader, data_type, result_filename, save_dir=None, show_image=True, frame_rate=30):
#     if save_dir:
...
#
#     video_time = video_name[8:10]
#     video_minute = video_name[10:12]
#     video_second = video_name[12:14]
#
#     #for path, img, img0 in dataloader:
#     for i, (path, time, img, img0) in enumerate(dataloader):
#         #if i % 8 != 0:
#             #continue
#         if frame_id % 20 == 0:
...
def eval_seq(opt, video_name, dataloader, data_type, result_filename, save_dir=None, show_image=True, frame_rate=30):
    ...
    #for path, img, img0 in dataloader:
    for i, (path, time, img, img0) in enumerate(dataloader):
        # Frame ID 0번 의미 없음 - 같은 Timestamp
        if frame_id == 0:
            frame_id = 1
            continue
        ...
        #if len(online_ids) == 0:
        #    data = str(timestamp + time) + ' ' + '0' + '\n'
        #else:
        #   data = str(timestamp + time) + ' ' + '1' + '\n'
        #print(data)
        #f.write(data)
