...
from function import syncVideo
...
class MultiLoadVideo:  # for inference
    def __init__(self, paths, sizes, filename):
        self.video_count = len(paths)
        self.filename = filename
        self.lv_list = [LoadVideo(i,j) for (i,j) in zip(paths,sizes)]
        self.frame_count = [0 for i in range(self.video_count)]
        self.count = 0
        self.frame_rate_list = [self.lv_list[i].frame_rate for i in range(self.video_count)]

        self.frame_rate_filter = []
        for frame in self.frame_rate_list:
            self.frame_rate_filter.append([1]*frame)
        # self.frame_rate_filter = syncVideo(self.frame_rate_filter)
    ...
    def __next__(self):
        self.count += 1
        count = self.count%self.video_count
        data = next(self.lv_list[count])
        # cv2.imwrite(img_path + '.letterbox.jpg', 255 * img.transpose((1, 2, 0))[:, :, ::-1])  # save letterbox image
        return self.count%self.video_count, data
